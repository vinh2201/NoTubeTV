...
/* Start speedMenuTrigger.js */
(function() {
    function applySpeedMod() {
        const speedBtn = document.querySelector('yt-icon.ytContribIconClockHalfCircle')?.closest('ytlr-button');
        if (speedBtn) {
            const speedBtnSub = speedBtn.querySelector('yt-formatted-string[idomkey="ytLrAvatarLockupSubtitle"]');
            if (speedBtnSub) {
                const customSub = document.createElement('span');
                customSub.textContent = configRead('videoSpeed') + 'x';
                customSub.className = speedBtnSub.className;
                speedBtnSub.replaceWith(customSub);
            }
            if (!speedBtn.dataset.listenerAttached) {
                speedBtn.dataset.listenerAttached = "true";
